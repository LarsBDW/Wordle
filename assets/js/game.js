(() => {
  ...
  const load=()=>{ const saved=JSON.parse(localStorage.getItem('gooseGame')||'null'); game=saved&&saved.answer&&saved.guesses?.length<6&&!saved.finished?saved:newGame(+localStorage.getItem('gooseLength')||6); render(); };
  const newGame=len=>({length:len,answer:GOOSE_TARGETS[len][Math.floor(Math.random()*GOOSE_TARGETS[len].length)],guesses:[],current:'',finished:false,keys:{}});
  ...
  function input(k){if(game.finished)return;if(k==='ENTER')return submit();if(k==='⌫'||k==='BACKSPACE'){game.current=game.current.slice(0,-1);render();return}if(/^[A-Z]$/.test(k)&&game.current.length<game.length){game.current+=k.toLowerCase();render()}}
  ...
  function confetti(){const box=$('#confetti');box.innerHTML=Array.from({length:80},(_,i)=>`<i style="--x:${Math.round(Math.random()*100)}%;--d:${.7+Math.random()*1.1}s;--r:${Math.round(Math.random()*500)}deg"></i>`).join('');box.classList.add('show');setTimeout(()=>{box.classList.remove('show');box.innerHTML=''},2300)}
